...
class _StallDetailScreenState extends State<StallDetailScreen> {
  ...
  Future<void> _refreshStallData() async {
    ...
    try {
      final stallDetails = await StallApiService.getStallDetails(
        widget.eventId,
        widget.zoneId,
        widget.trackId,
        widget.stallId,
      ...
      if (mounted) {
        setState(() {
          _currentStallData = stallDetails;
          _isLoading = false;
        });
      }
    } catch (e) {
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error refreshing stall data: $e')),
        );
      }
    }
  }
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                await StallApiService.updateStall(
                  widget.eventId,
                  widget.zoneId,
                  widget.trackId,
                  widget.stallId,
